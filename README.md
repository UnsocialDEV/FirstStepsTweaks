...
The current runtime is centered on a small [`FirstStepsTweak.cs`](./FirstStepsTweaks/FirstStepsTweak.cs) entry point that loads config, registers privileges, builds shared runtime dependencies, and wires feature modules.
...
The server entry point is [`FirstStepsTweak.cs`](./FirstStepsTweaks/FirstStepsTweak.cs), which contains the `FirstStepsTweaks : ModSystem` class.
...
Inside the project folder, representative files include:

- `Commands/BackCommands.cs`
- `Commands/DiscordLinkCommands.cs`
- `Commands/StormShelterCommands.cs`
- `Features/ChatFeature.cs`
- `Features/DiscordFeature.cs`
- `Features/TeleportFeature.cs`
- `Services/PlayerDonatorRoleSyncService.cs`
- `Services/LandClaimEscapeService.cs`
- `Teleport/HomeStore.cs`
- `Discord/DiscordLinkPoller.cs`
...
The main config object is `FirstStepsTweaksConfig` in [`Config/FirstStepsTweaksConfig.cs`](./FirstStepsTweaks/Config/FirstStepsTweaksConfig.cs).
...
The gravestone system is intentionally decomposed. The higher-level `GravestoneService` coordinates smaller components under [`Gravestones/`](./FirstStepsTweaks/Gravestones), including:
...
```powershell
dotnet build .\FirstStepsTweaks\FirstStepsTweaks.csproj -p:VintagestoryApiPath="C:\path\to\VintagestoryAPI.dll"
```

### Test

```powershell
dotnet test .\FirstStepsTweaks\Tests\FirstStepsTweaks.Tests\FirstStepsTweaks.Tests.csproj -p:VintagestoryApiPath="C:\path\to\VintagestoryAPI.dll"
```

### Packaging

Packaging is opt-in and runs after `dotnet publish` by calling [`scripts/CreateModZip.ps1`](./FirstStepsTweaks/scripts/CreateModZip.ps1).
...
```powershell
dotnet publish .\FirstStepsTweaks\FirstStepsTweaks.csproj `
  -c Release `
  -r linux-x64 `
...
```powershell
dotnet build .\FirstStepsTweaks\FirstStepsTweaks.csproj `
  -p:VintagestoryApiPath="C:\path\to\VintagestoryAPI.dll" `
